...
if not TOPIC_ID_STR:
    raise ValueError("❌ TOPIC_ID не найден!")
TOPIC_ID = int(TOPIC_ID_STR)

ADMIN_IDS_STR = os.getenv("ADMIN_IDS")
if not ADMIN_IDS_STR:
    raise ValueError("❌ ADMIN_IDS не найден!")
...
def save_user_info(user):
    try:
        with sqlite3.connect("garage.db") as conn:
            conn.execute("INSERT OR REPLACE INTO users (user_id, username, full_name) VALUES (?, ?, ?)",
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, что у сообщения есть thread_id (это топик)
...
async def cars_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # В ЛС — только админка для админов
...
# ... (остальные функции без изменений, я их сократил для читаемости, но в реальном коде они все должны быть)
# ... (show_free_cars_in_topic, show_user_taken_cars_in_topic, ask_car_condition_in_topic, take_car, return_car, show_history, admin_panel, add_car_name, add_car_plate, cancel, show_cars_for_remove, remove_car, show_taken_cars_for_admin, force_return_car, back_to_menu)
